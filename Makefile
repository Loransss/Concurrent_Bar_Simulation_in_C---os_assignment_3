...
# Default target
all: $(MAIN_EXEC) $(MONITOR_EXEC) $(VISITOR_EXEC) $(RECEPTIONIST_EXEC) $(STATISTICS_EXEC)  $(TEST_EXEC)
...
# Build statistics executable
$(STATISTICS_EXEC): $(STATISTICS_SRC) $(SHARED_MEMORY_OBJ)
	$(CC) $(CFLAGS) -o $(STATISTICS_EXEC) $(STATISTICS_SRC) $(SHARED_MEMORY_OBJ)

# Build test executable
$(TEST_EXEC): 
	$(CC) -o $(TEST_EXEC) $(TEST_SRC) -pthread



# Clean target
clean:
	rm -f $(MAIN_EXEC) $(MONITOR_EXEC) $(VISITOR_EXEC) $(RECEPTIONIST_EXEC) $(SHARED_MEMORY_OBJ) $(STATISTICS_EXEC) $(TEST_EXEC) bar_key.txt 
